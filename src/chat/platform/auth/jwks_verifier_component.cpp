...
std::vector<unsigned char> Base64UrlDecode(std::string_view encoded) {
  ...
  const auto written =
      EVP_DecodeBlock(decoded.data(), reinterpret_cast<const unsigned char*>(normalized.data()),
...
JwtParts SplitJwt(std::string_view token) {
  ...
  if (first == std::string_view::npos || second == std::string_view::npos ||
      token.find('.', second + 1) != std::string_view::npos || first == 0 || second == first + 1 ||
      second + 1 == token.size()) {
...
EvpPkeyPtr BuildRsaPublicKey(std::string_view modulus, std::string_view exponent) {
  ...
  BnPtr n(BN_bin2bn(n_bytes.data(), static_cast<int>(n_bytes.size()), nullptr), BN_free);
  BnPtr e(BN_bin2bn(e_bytes.data(), static_cast<int>(e_bytes.size()), nullptr), BN_free);
  ...
  if (!n || !e || !builder || !context ||
      OSSL_PARAM_BLD_push_BN(builder.get(), OSSL_PKEY_PARAM_RSA_N, n.get()) != 1 ||
...
bool AudienceMatches(const userver::formats::json::Value& audience, std::string_view expected) {
...
std::string CanonicalUuid(const userver::formats::json::Value& value) {
  return userver::utils::ToString(userver::utils::BoostUuidFromString(value.As<std::string>()));
}
...
class JwksVerifierComponent::Impl final {
 public:
  ...
  AccessTokenClaims Verify(std::string_view token) const {
    try {
      ...
      const auto header = userver::formats::json::FromString(DecodeText(parts.header));
      ...
      const auto payload = userver::formats::json::FromString(DecodeText(parts.payload));
      ...
      if (!payload["nbf"].IsMissing() && payload["nbf"].As<std::int64_t>() > now) {
 ...
 private:
  ...
  void RefreshKeys() const {
    try {
      const auto response =
          http_client_.CreateRequest().get(jwks_url_).retry(2).timeout(request_timeout_).perform();
      response->raise_for_status();
      const auto document = userver::formats::json::FromString(response->body_view());

      std::unordered_map<std::string, EvpPkeyPtr> refreshed;
      for (const auto& jwk : document["keys"]) {
        if (jwk["kty"].As<std::string>() != "RSA" || jwk["alg"].As<std::string>() != "RS256") {
          continue;
        }
        refreshed.emplace(
            jwk["kid"].As<std::string>(),
            BuildRsaPublicKey(jwk["n"].As<std::string>(), jwk["e"].As<std::string>()));
      }
      if (refreshed.empty()) {
      ...
      throw JwksUnavailable(std::string{"failed to refresh JWKS: "} + error.what());
    }
  }
...
JwksVerifierComponent::JwksVerifierComponent(const userver::components::ComponentConfig& config,
                                             const userver::components::ComponentContext& context)
    : ComponentBase(config, context), impl_(std::make_unique<Impl>(config, context)) {}
